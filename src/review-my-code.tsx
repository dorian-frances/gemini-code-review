...
function getBranchDiffAndHandleError(reviewParameters: ReviewParameters): string {
  try {
    return getGitDiff(reviewParameters.repository.repositoryPath, reviewParameters.currentBranch, reviewParameters.targetBranch);
  } catch (error) {
    showToast({
...
export default function Command() {
  ...
  if (modelReview !== "" && reviewParameterState)  {
    return <Detail isLoading={loadingModelReview} markdown={`# Code review from Gemini for branch \`${reviewParameterState.currentBranch}\`\n${modelReview}`} />;
  }
